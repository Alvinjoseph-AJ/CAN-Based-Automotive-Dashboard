...
void speed() {
    ...
    uint16_t speed = read_adc(0x04) / 10.33;
    can_transmit(0x10, &speed, sizeof (speed));
}
...
void gearshift() {
    ...
    can_transmit(0x20, &gear, sizeof (gear));
}

void temp() {
    int digit = read_adc(0x06);
    float volts = (float) digit * 0.004887;
    uint16_t temperature = ((volts * 1000) / 10);
    can_transmit(0x40, &temperature, sizeof (temperature));
}
